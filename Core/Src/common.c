...
int fputc(int ch, FILE *f)
{
    ...
    static uint8_t printf_idx = 0;

    // 将接收到的字符存入缓冲区
    printf_buf[printf_idx++] = (uint8_t)ch;

    // 如果遇到换行符，或者缓冲区即将溢出，则将整个缓冲区的数据打包发送
    if ((ch == '\n') || (printf_idx >= 127))
    {
        // 使用项目自带的非阻塞发送函数，将整个缓冲区作为一帧数据放入队列
        UartTxDataQueue(1, (uint8_t*)printf_buf, printf_idx);
        // 发送完毕后，清空缓冲区索引
        printf_idx = 0;
    }
