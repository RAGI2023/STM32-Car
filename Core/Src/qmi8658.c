...
SPI_HandleTypeDef * g_qmispi = &hspi2;
__IO uint8_t  g_qmi8658_flag = 0 ;
...
#define LED1_OFF    HAL_GPIO_WritePin(TX_LED1_GPIO_Port,TX_LED1_Pin,GPIO_PIN_RESET);
#define LED1_ON     HAL_GPIO_WritePin(TX_LED1_GPIO_Port,TX_LED1_Pin,GPIO_PIN_SET);
#define LED1_TOGGLE HAL_GPIO_TogglePin(TX_LED1_GPIO_Port,TX_LED1_Pin);
...
uint8_t qst_imu_spi_write(uint8_t reg, uint8_t value)
{
  ...
  HAL_SPI_Transmit(g_qmispi, (uint8_t *)&reg, 1, 20);
  HAL_SPI_Transmit(g_qmispi, (uint8_t *)&value, 1, 20);
//  HAL_SPI_Receive(&hspi1, buf, len, 20);
...
uint8_t qst_imu_spi_write_bytes(uint8_t reg, uint8_t *value, uint8_t len)
{
  ...
  HAL_SPI_Transmit(g_qmispi, (uint8_t *)&reg, 1, 20);
  HAL_SPI_Transmit(g_qmispi, (uint8_t *)value, len, 20);
//  HAL_SPI_Receive(&hspi1, buf, len, 20);
...
uint8_t qst_8658_spi_read(uint8_t reg, uint8_t *buf, uint16_t len)
{
  reg = reg | 0x80;
  ...
  HAL_SPI_Transmit(g_qmispi, (uint8_t *)&reg, 1, 20);
  HAL_SPI_Receive(g_qmispi, (uint8_t *)buf, len, 20);
...
void qst_delay_us(uint32_t delay)
{
...
void qmi8658_do_hw_selftest(int enSensor)
{
	...
	if(enSensor & QMI8658_ACC_ENABLE)
	{
		...
		while(!(status_int & 0x01))
		{
			...
			if(retry++ > 5000)
			{
				qmi8658_log("wait int high timeout\r\n");
				break;
			}
		}
		...
		while((status_int & 0x01))
		{
			...
			if(retry++ > 5000)
			{
				qmi8658_log("wait int low timeout\r\n");
				break;
			}
		}
